...
import type { IAdminSchemaEdit } from '@modules/admin/infrastructure/interfaces/admin.interface';

export class EditAdminUseCase {
  @DefaultCatch(defaultCatchError)
  @Catch(AxiosError, axiosCatchError)
  @Catch(ZodError, zodCatchError)
  static async handle(data: IAdminSchemaEdit, id: number) {
    const authStore = useAuthStore();

    const result = await AdminGateway.editAdmin(
      data,
      id,
      authStore.GetToken,
      authStore
